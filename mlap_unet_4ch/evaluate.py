...
import utils
from model import LaplacianLayer
...
def evaluate():
    log.info("--------------------------------------------------")
    log.info("           EVALUATION DEEP MEAN LAP U-NET         ")
    log.info("--------------------------------------------------")

    # UPDATED MODEL PATH
    model_path = os.path.join(config.CHECKPOINT_DIR, config.MODEL_NAME_BEST)
    if not os.path.exists(model_path):
        log.error("Model not found. Please train the Deep mlap_unet first.")
        return
    ...
    for scan_id in tqdm(test_ids, desc="Evaluating"):
        try:
            ...
            if scan_id in TARGET_PATIENTS:
                save_name = f"{scan_id}_mlap_unet_cleaned.png"
                utils.visualize_inference(
                    input_stack, 
                    gt_mask,
                    y_pred_cleaned,
                    title_suffix=f"({scan_id} - MLAP_UNET)",
                    save_name=save_name
                )
