...
def main():
    log.info("--------------------------------------------------")
    log.info("              MLAP-UNET (1024 FILTER)             ")
    log.info("--------------------------------------------------")
    ...
    log.info("Building 4-Channel MLAP-UNet Model...")
    strategy = tf.distribute.MirroredStrategy()
    with strategy.scope():
        model = build_unet()
        
        # Save Model Summary
        summary_path = os.path.join(config.RESULTS_DIR, "model_summary_mlap_unet.txt")
    ...
    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(config.CHECKPOINT_DIR, config.MODEL_NAME_BEST),
        save_best_only=True,
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(os.path.join(config.LOGS_DIR, "training_log_mlap_unet.csv"))
    ...
    save_path = os.path.join(config.CHECKPOINT_DIR, config.MODEL_NAME_BEST)
