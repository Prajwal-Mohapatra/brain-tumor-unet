...
def evaluate():
    log.info("--------------------------------------------------")
    log.info("    PHASE 3: LAPLACIAN U-NET EVALUATION           ")
    log.info("--------------------------------------------------")

    model_path = os.path.join(config.CHECKPOINT_DIR, "laplacian_unet_best.keras")
    if not os.path.exists(model_path):
        log.error("Model not found. Please train Phase 3 first.")
        return
        ...
        try:
            ...
            for i, p_mod in enumerate(mod_paths):
                ...
                if len(batch_metrics_list) <= (5 * 4): 
                    save_name = f"{scan_id}_{mod_name}_lap_pred.png"
                    utils.visualize_inference(
                        input_tensor,
                        gt_mask,
                        y_pred_int,
                        title_suffix=f"({scan_id} - {mod_name} - Laplacian)",
                        save_name=save_name
                    )
    ...
    print("\n" + "="*50)
    print("FINAL PHASE 3 TEST METRICS (Laplacian U-Net)")
    print("="*50)
