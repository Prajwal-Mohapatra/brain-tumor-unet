...
def main():
    log.info("--------------------------------------------------")
    log.info("        PHASE 2: DUAL ATTENTION TRAINING          ")
    log.info("--------------------------------------------------")
    ...
    log.info("Building Dual Attention U-Net Model...")
    strategy = tf.distribute.MirroredStrategy()
    with strategy.scope():
        model = build_unet()
        
        # Save Model Summary
        summary_path = os.path.join(config.RESULTS_DIR, "model_summary_dual_attention.txt")
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(os.path.join(config.LOGS_DIR, "training_log_phase2.csv"))
