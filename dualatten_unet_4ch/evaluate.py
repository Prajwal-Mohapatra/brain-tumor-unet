...
def evaluate():
    log.info("--------------------------------------------------")
    log.info("           EVALUATION DEEP DAU-NET                ")
    log.info("--------------------------------------------------")

    # UPDATED MODEL PATH
    model_path = os.path.join(config.CHECKPOINT_DIR, "att_unet_brats_best.keras")
    if not os.path.exists(model_path):
        log.error("Model not found. Please train the Deep Dual Att-UNet first.")
        return
    ...
    for scan_id in tqdm(test_ids, desc="Evaluating"):
        try:
            ...
            if scan_id in TARGET_PATIENTS:
                save_name = f"{scan_id}_dau-net_cleaned.png"
                utils.visualize_inference(
                    input_stack, 
                    gt_mask,
                    y_pred_cleaned,
                    title_suffix=f"({scan_id} - DAU-Net)",
                    save_name=save_name
                )
