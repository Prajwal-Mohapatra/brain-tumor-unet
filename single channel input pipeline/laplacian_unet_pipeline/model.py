...
class LaplacianLayer(Layer):
    ...
    Uses a fixed 3x3 Laplacian kernel.
    """
    ...
    def call(self, inputs):
        # Apply depthwise conv to treat each channel independently
        # If input has N channels, we replicate the kernel N times
        # However, for simplicity in single-channel input, standard conv works.
        
        # We assume inputs are (B, H, W, C).
        # We iterate over channels or use DepthwiseConv2D logic manually if needed.
        # Since our input is typically 1 channel (from config), this is straightforward.
        
        # Create Laplacian edges
        edges = tf.nn.conv2d(inputs, self.kernel, strides=[1, 1, 1, 1], padding='SAME')
        return edges
...
def build_unet():
    ...
    1. Input Branch: Original Image
    2. Laplacian Branch: Edge Map of Image
    3. Fusion: Concatenate Input + Edges before Encoder
    ...
    # Extract edges from raw input to guide the network
    edges = LaplacianLayer()(inputs)
    
    # 2. Early Fusion
    # Merge original intensity with edge information
