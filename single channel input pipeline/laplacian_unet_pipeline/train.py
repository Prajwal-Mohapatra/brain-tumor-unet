...
def main():
    log.info("--------------------------------------------------")
    log.info("    PHASE 3: MEAN-ENABLED LAPLACIAN U-NET         ")
    log.info("--------------------------------------------------")
    ...
    log.info("Building Laplacian U-Net Model...")
    strategy = tf.distribute.MirroredStrategy()
    with strategy.scope():
        model = build_unet()
        
        # Save Model Summary
        summary_path = os.path.join(config.RESULTS_DIR, "model_summary_laplacian.txt")
    ...
    # Callbacks - Updated Names for Phase 3
    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(config.CHECKPOINT_DIR, "laplacian_unet_best.keras"),
        save_best_only=True,
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(os.path.join(config.LOGS_DIR, "training_log_phase3.csv"))
    ...
    save_path = os.path.join(config.CHECKPOINT_DIR, "laplacian_unet_final.keras")
    model.save(save_path)
    
    log.info("Generating Training Plots...")
    plot_training_history(history)
    log.info("Phase 3 Pipeline Finished Successfully.")
